...
def validate_fields():
    ...
    if 20 < len(pw1) < 3:
        pw1 = pw1
        pw1_error = 'The first Password must contain more than 3 characters long, 20 max, minimum 14 recommended'

    if 20 < len(pw2) < 3:
        pw2 = pw2
        pw2_error = 'The second Password must contain more than 3 characters long, 20 max, minimum 14 recommended'    
    ...
    if pw1 == "":
        pw1empty_error = 'Your first password field was left empty. Set a password, no fewer than 3 and no longer than 20 characters'
    if pw2 == "":
        pw2empty_error = 'Your second password field was left empty. Enter a password to match the one above, no fewer than 3 and no longer than 20 characters'


    if not username_error and not pw1_error and not pw2_error and not pw_error and not email_error and not pw1empty_error and not pw2empty_error:
